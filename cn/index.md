---
title: 星合工作室
layout: default
---

# Constellar Studio 星合工作室

星合工作室致力于利用卫星与机载雷达数据研究地表变化。我们提供基础设施监测、地表形变与变化检测，以及地球内部结构三维重建等相关服务。

我们的工作涵盖多源合成孔径雷达（SAR）数据处理，包括卫星与机载平台数据。我们分析不同空间与时间尺度下的地表形变，支持识别道路、桥梁、建筑等关键基础设施的潜在风险。同时，我们也为野火、洪水、滑坡以及多年冻土退化等自然灾害提供早期监测与环境咨询服务。

通过对雷达数据的系统分析，我们希望为工程建设、环境保护与土地管理提供决策支持，降低风险，并加深对自然环境与人类活动区域变化的理解。

## 我们可以做什么

- 利用卫星与机载 SAR 数据分析地表变化  
- 多时空尺度下的地表形变监测与变化检测  
- 道路、桥梁、建筑等关键基础设施风险识别  
- 野火、洪水、滑坡及多年冻土等自然灾害监测与环境咨询  

<hr class="section-divider">

<h2 class="customers-title">合作客户</h2>

<div class="customer-grid">
  <div class="customer-item">
    <img src="/assets/images/cooperations/coop_air.png" class="customer-logo" alt="中国科学院空天信息研究院">
    <span class="customer-name">中国科学院<br>空天信息研究院</span>
  </div>
  <div class="customer-item">
    <img src="/assets/images/cooperations/coop_caf.png" class="customer-logo" alt="中国林业科学研究院">
    <span class="customer-name">中国林业<br>科学研究院</span>
  </div>
  <div class="customer-item">
    <img src="/assets/images/cooperations/coop_casm.png" class="customer-logo" alt="中国测绘科学研究院">
    <span class="customer-name">中国测绘<br>科学研究院</span>
  </div>
  <div class="customer-item">
    <img src="/assets/images/cooperations/coop_tjuni.png" class="customer-logo" alt="天津大学">
    <span class="customer-name">天津大学</span>
  </div>
  <div class="customer-item">
    <img src="/assets/images/cooperations/coop_unioftexas.png" class="customer-logo" alt="美国德克萨斯州立大学">
    <span class="customer-name">美国德克萨斯<br>州立大学</span>
  </div>
</div>